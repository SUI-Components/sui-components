...
const ArticleDefault = ({className}) => {
  ...
  return (
    <Article className={className} mode={mode}>
      <H2>Type</H2>
      <Paragraph>
        using the prop <Code>type</Code> user can inherit the helpText color styler for validation text usages.
      </Paragraph>
      <RadioButton onClick={() => setMode(mode === 'light' ? 'dark' : 'light')} label={mode} />
      <br />
      <br />
