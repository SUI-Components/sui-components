...
import AtomVideoPlayer, {
  AUTOPLAY
} from 'components/atom/videoPlayer/src/index.js'
...
const DemoPlayer = ({src}) => {
  ...
  return (
    <>
      ...
      <RadioButtonGroup
        value={autoPlay}
        onChange={(event, value) => setAutoPlay(value)}
      >
        <RadioButton value={AUTOPLAY.TRUE} label={'Enabled'} />
        <RadioButton value={AUTOPLAY.FALSE} label={'Disabled'} />
        <RadioButton
          value={AUTOPLAY.ON_VIEWPORT}
          label={'On viewport intersection'}
        />
      </RadioButtonGroup>
      <H2>Controls</H2>
      <RadioButtonGroup
        value={controls}
        onChange={(event, value) => setControls(value)}
      >
      ...
      <RadioButtonGroup
        value={muted}
        onChange={(event, value) => setMuted(value)}
      >
        <RadioButton value={true} label={'Enabled'} />
        <RadioButton value={false} label={'Disabled'} />
      </RadioButtonGroup>
