...
import {Article, Box, Cell, Grid, H2, Paragraph} from '@s-ui/documentation-library'
...
const ArticleGridDemo = ({className}) => {
  ...
  return (
    <Article className={className}>
      ...
      <Paragraph>There are some strategies to stack many sticky elements beside each other.</Paragraph>
      <BehaviorStickyProvider>
        <Box style={{padding: 0}}>
          <Grid gutter={[8, 8]} cols={Object.values(COLORS).length} className={CLASS_DEMO_BACKGROUND}>
            {Object.entries(COLORS).map(([colorKey, colorValue], index) => (
              <Cell key={colorKey} ref={setContainerRef(index)} style={getElementStyles(getBoxRefs(), index)}>
      ...
      <Paragraph>
        When the container referenced element targeted by the Sticky does NOT have enough space to contain its children
        in the viewport it is placed to its natural dom position.
      </Paragraph>
    </Article>
  )
}
