...
const loop = (n, {setRef, getRef, getRefs}, {setStackRef, getStackRefs}) => {
  const recursion = index => {
    ...
    return (
      <Box ref={setRef(index)} style={{padding: 0, overflowY: 'hidden', borderWidth: 0}}>
...
const ArticleStacked = ({className}) => {
  ...
  return (
    <Article className={className}>
      ...
      <Paragraph>
        There are many options to stack many sticky elements at the top giving them a margin-top with the previous
        accumulated height.
      </Paragraph>
      <BehaviorStickyProvider>
        {loop(Object.values(COLORS).length - 1, {setRef, getRef, getRefs}, {getStackRef, setStackRef, getStackRefs})}
      </BehaviorStickyProvider>
    </Article>
  )
}
