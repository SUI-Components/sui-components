...
export const AtomListFeaturedItem = function (props) {
  const featuredRemoveId = BASE_CLASS + '_' + Math.floor(Math.random() * 10000)
  const {option, onClickCallback, onFocusCallback, onRemoveFeatureItem, iconFeatured, iconRemove} = props
  ...
  const iconFeaturedComponentResolved = (iconFeatured) || IconFeatured
  const iconRemoveComponentResolved = (iconRemove) || IconRemove

  const onclickwrapper = (option) => (event) => {
    if (document.getElementById(featuredRemoveId).contains(event.target)) {
      onRemoveFeatureItem(option)()
    } else {
  ...
  return (
    <div
      ...
      onClick={onclickwrapper(option)}
      onMouseOver={onFocusCallback(option)}
    >
      <span className='icon-left'>
        {iconFeaturedComponentResolved()}
      </span>
      <span className='text'>{option.getStringTruncated()}</span>
      <div className='icon-right' id={featuredRemoveId}>
        {iconRemoveComponentResolved()}
      </div>
    </div>
  )
}
...
AtomListFeaturedItem.propTypes = {
  /**
   * Featured option to be shown following model OptionShown
   */
   ...
   */
  iconRemove: PropTypes.any,
  /**
   * Icon on a function returning rxjs. This icon is to mark a featured item
   */
  iconFeatured: PropTypes.any,

}

const IconFeatured = function () {
  return (
        ...
        <g>
          <g>
            <g>
              ...
              <g mask='url(#mask-2)' fill='#777777' fillRule='nonzero'>
