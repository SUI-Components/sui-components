...
const ArticleCustomIcon = ({className, items}) => {
  ...
  return (
    <Article className={className}>
      ...
      <MoleculeBreadcrumb ariaLabelText="breadcrumb" items={items} icon={getCustomIcon(iconID)} />
      <Paragraph>Icon might be even a text (or string) Component node</Paragraph>
      <Input value={text} onChange={event => setText(event.target.value)} />
      <MoleculeBreadcrumb ariaLabelText="breadcrumb" items={items} icon={getCustomTextIcon(text)} />
    </Article>
  )
}
