...
const ArticleSize = ({className}) => {
  return (
    <Article className={className}>
      <H2>Size</H2>
      <Paragraph>
        There are {Object.values(moleculeButtonGroupSizes).length} different designs available for grouping atomButtons.
        They are provided by exported <Code>moleculeButtonGroupSizes</Code> enum:
      </Paragraph>
      <UnorderedList>
        {Object.entries(moleculeButtonGroupSizes).map(([moleculeButtonGroupSizeKey, moleculeButtonGroupSizeValue]) => (
      ...
      <Grid gutter={[8, 8]} cols={Object.values(moleculeButtonGroupSizes).length}>
        {Object.entries(moleculeButtonGroupSizes).map(([moleculeButtonGroupSizeKey, moleculeButtonGroupSizeValue]) => (
