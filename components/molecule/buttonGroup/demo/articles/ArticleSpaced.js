...
const ArticleSpaced = ({className}) => {
  return (
    <Article className={className}>
      <H2>Spaced</H2>
      <Paragraph>
        There are {Object.values(moleculeButtonGroupSpaced).length} types of
        spacing between items. They are provided by exported{' '}
        <Code>moleculeButtonGroupSpaced</Code> enum:
      </Paragraph>
      <UnorderedList>
      ...
      <Grid
        gutter={[8, 8]}
        cols={Object.values(moleculeButtonGroupSpaced).length}
      >
        {Object.entries(moleculeButtonGroupSpaced).map(
