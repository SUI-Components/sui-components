...
const ArticleFullWidth = ({className}) => (
  <Article className={className}>
    <H2>FullWidth</H2>
    <Paragraph>
      By default, slides uses the full width of the container. So, if you're
      using an image, it uses all the width available. You could avoid this by
      using the prop <Code>isFullWidth</Code>.
    </Paragraph>
    ...
    <Paragraph>
      If you have slides with different heights you need to specify the
      maxHeight to be used for each slide, in order to avoid inner images or
      content to make the slider jump as they will automatically adapt to the
      available width and automatically detect the height.
    </Paragraph>
