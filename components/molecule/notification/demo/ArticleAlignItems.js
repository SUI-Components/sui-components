...
const ArticleAlignItems = ({className}) => {
  return (
    <Article className={className}>
      <H2>Align Items</H2>
      <Paragraph>
        Items in the notification can be vertically aligned with the{' '}
        <Code>alignItems</Code> (enum) prop
      </Paragraph>
      <UnorderedList>
      ...
      <Grid
        gutter={[8, 8]}
        cols={Object.keys(moleculeNotificationAlignItems).length + 1}
      >
        {[
          ['undefined', undefined],
          ...Object.entries(moleculeNotificationAlignItems)
        ].map(
          ([
            moleculeNotificationAlignItemsKey,
            moleculeNotificationAlignItemsValue
            ...
            <Cell key={moleculeNotificationAlignItemsKey}>
              <MoleculeNotification
                autoClose="manual"
                alignItems={moleculeNotificationAlignItemsValue}
              >
                {`${moleculeNotificationAlignItemsValue}`}
              </MoleculeNotification>
            </Cell>
