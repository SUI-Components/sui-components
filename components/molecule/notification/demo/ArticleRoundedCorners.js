...
import MoleculeNotification, {moleculeNotificationBorderSizes} from '../src/index.js'
...
const ArticleRoundedCorners = ({className}) => {
  return (
    <Article className={className}>
      ...
      <Paragraph>
        The Notification borders can be rounded using the <Code>roundedBorders</Code> (enum) prop
      </Paragraph>
      <UnorderedList>
        {Object.entries(moleculeNotificationBorderSizes).map(
          ([moleculeNotificationRoundedCornerKey, moleculeNotificationRoundedCornerValue]) => (
      ...
      <Grid gutter={[8, 8]} cols={Object.keys(moleculeNotificationBorderSizes).length + 1}>
        {[['undefined', undefined], ...Object.entries(moleculeNotificationBorderSizes)].map(
          ([moleculeNotificationRoundedCornerKey, moleculeNotificationRoundedCornerValue]) => (
            <Cell key={moleculeNotificationRoundedCornerKey}>
              <MoleculeNotification autoClose="manual" roundedCorners={moleculeNotificationRoundedCornerValue}>
                {`${moleculeNotificationRoundedCornerValue}`}
