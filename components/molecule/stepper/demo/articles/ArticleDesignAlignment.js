...
const ArticleDesignAlignment = ({className}) => {
  ...
  return (
    <Article className={className}>
      ...
      <Paragraph>
        There are {Object.values(moleculeStepperDesign).length} different
        designs:
      </Paragraph>
      <UnorderedList>
        {Object.entries(moleculeStepperDesign)
          .map(([moleculeStepperDesignKey, moleculeStepperDesignValue]) => {
            const text = {
              DEFAULT:
                'Labeled and connected, commonly used for desktop purposes.',
              COMPRESSED:
            ...
            return [
              moleculeStepperDesignKey,
              moleculeStepperDesignValue,
              text[moleculeStepperDesignKey]
            ]
          })
          .map(
            ([moleculeStepperDesignKey, moleculeStepperDesignValue, text]) => (
              <ListItem key={moleculeStepperDesignKey}>
                <Paragraph>
                  <Code>{moleculeStepperDesignKey}</Code> (
                  {moleculeStepperDesignValue}): {text}
                </Paragraph>
              </ListItem>
            )
          )}
      ...
      <Paragraph>
        There are {Object.values(moleculeStepperAlignment).length} different
        alignments:
      </Paragraph>
      <UnorderedList>
        {Object.entries(moleculeStepperAlignment).map(
          ([moleculeStepperAlignmentKey, moleculeStepperAlignmentValue]) => (
            <ListItem key={moleculeStepperAlignmentKey}>
              <Paragraph>
                <Code>{moleculeStepperAlignmentKey}</Code> (
      ...
      <Grid
        cols={Object.values(moleculeStepperDesign).length + steps + 1}
        gutter={[8, 8]}
      >
        {Object.entries(moleculeStepperAlignment).map(
          ([moleculeStepperAlignmentKey, moleculeStepperAlignmentValue]) => {
            const child = {
              VERTICAL: (
                <Cell
                  key={moleculeStepperAlignmentKey}
                  span={Object.values(moleculeStepperDesign).length}
                >
                  <H3>{moleculeStepperAlignmentValue}</H3>
                  <Grid
                    cols={Object.values(moleculeStepperDesign).length}
                    gutter={[0, 8]}
                  >
                    {Object.entries(moleculeStepperDesign).map(
                      ([, moleculeStepperDesignValue], i) => (
      ...
      <Paragraph>
        Use the controls to check your own design-aligment combination
      </Paragraph>
      <Grid gutter={[8, 8]} cols={2}>
        ...
        <Cell>
          <RadioButtonGroup
            ...
            fullWidth
          >
            {[undefined, ...Object.values(moleculeStepperAlignment)].map(
              moleculeStepperAlignmentValue => (
                <RadioButton
        ...
        <Cell>
          <RadioButtonGroup
            value={design}
            onChange={(event, value) => setDesign(value)}
            fullWidth
          >
            {[undefined, ...Object.values(moleculeStepperDesign)].map(
              moleculeStepperDesignValue => (
                <RadioButton
