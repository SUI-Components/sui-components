...
import MoleculeTabs, {MoleculeTab, moleculeTabsTypes} from 'components/molecule/tabs/src/index.js'
...
const ArticleDefault = ({className}) => {
  ...
  return (
    <Article className={className}>
      <H2>Type</H2>
      <Paragraph>
        Using the <Code>type</Code> (enum) prop you can set it to any of the predefined types. The default type values
        are exported on an enum called <Code>moleculeTabsTypes</Code>:
      </Paragraph>
      <UnorderedList>
        {Object.entries(moleculeTabsTypes).map(([moleculeTabsTypeKey, moleculeTabsTypeValue]) => (
          <ListItem key={moleculeTabsTypeKey}>
            <Code>moleculeTabsTypes.{moleculeTabsTypeKey}</Code>: "{moleculeTabsTypeValue}"
          </ListItem>
        ))}
      </UnorderedList>
