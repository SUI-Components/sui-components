...
import MoleculeTabs, {MoleculeTab, moleculeTabsTypes, moleculeTabsVariants} from 'components/molecule/tabs/src/index.js'
...
const ArticleVariant = ({className}) => {
  ...
  return (
    <Article className={className}>
      <H2>Variant</H2>
      <Paragraph>
        Using the <Code>variant</Code> (enum) prop you can set it to any of the predefined variants. The default type
        values are exported on an enum called <Code>moleculeTabsVariants</Code>:
      </Paragraph>
      <UnorderedList>
        {Object.entries(moleculeTabsVariants).map(([moleculeTabsVariantKey, moleculeTabsVariantValue]) => (
          <ListItem key={moleculeTabsVariantKey}>
            <Code>moleculeTabsVariants.{moleculeTabsVariantKey}</Code>: "{moleculeTabsVariantValue}"
          </ListItem>
        ))}
      </UnorderedList>
