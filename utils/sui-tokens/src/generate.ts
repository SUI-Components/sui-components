...
export const generate = {
  native: (
    ...
    const add = (partial: any, keyword: string, prefix: string, ident: number = 0) => {
      ...
      const getTokenKey = (key: string) => `--${prefix === '' ? '' : `${prefix}-`}${key}`
...
$${key}: var(${getTokenKey(key)}) !default;`
    ...
    const {prefix: prefixSettings, fontSize} = settings
    const prefix = prefixSettings === undefined ? '' : prefixSettings
...
`
  ...
  scss: (
    ...
    const add = (partial: any, keyword: string, prefix: string, ident: number = 0) => {
      ...
      const getTokenKey = (key: string) => `--${prefix === '' ? '' : `${prefix}-`}${key}`
      const getSCSSKey = (key: string) => `${prefix === '' ? '' : `${prefix}-`}${key}`
...
${identStr}${getTokenKey(key)}: ${value};`
      ...
      scssTokens[keyword] = String().concat(
        ...Array.from(maps[keyword]).map(val => {
          const [key] = val as [string, string | number]
          return `
$${getSCSSKey(key)}: var(${getTokenKey(key)}) !default;`
        })
      )
    }

    const {prefix: prefixSettings} = settings
    const prefix = prefixSettings === undefined ? '' : prefixSettings
    const hasMode = (mode?: 'light' | 'dark') => mode !== undefined
    ...
    return `/* This file is auto-generated by sui-tokens */
${scssTokens.color}
${hasPrimitive ? `${scssTokens.primitiveColor}\n` : ''}${scssTokens.font}
${scssTokens.opacity}
